...
## Sanity Check Calculations and Plots

This part will only operate on mice that have been tested a certain number of times.  
...
This section will produce the followin files
...
fwrite(d5,paste("../PSY_Processed/Mean_Strain_TestNo_",Dose, format(Sys.time(),"%Y-%m-%d(%H_%M%p)"),".csv", sep = ""))
fwrite(d6,paste("../PSY_Processed/SEM_Strain_TestNo_",Dose, format(Sys.time(),"%Y-%m-%d(%H_%M%p)"),".csv", sep = ""))
...
```
## Plot the Sens Data.
I'm recycling code from previous Sens Analysis.
Using File d3 from above chunk
Plots will appear in "Sensitization Analysis*.pdf" file
...
sp1 <- ggplot(aes(x = as.factor(TestNumber),y = sum5minavg, colour = strainName,group = strainName), data=d3) +
  ...
  geom_line(size = 1.0, stat = "summary") +
...
sp1 <- ggplot() +
  ...
  geom_line(aes(x = as.factor(TestNumber),y = sum5minavg, group = strainName), data=d3, size = 1.0, stat = "summary") +
...
sp1 <- ggplot() +
  ...
  geom_point(aes(x = as.factor(TestNumber),y = sum60min, colour = as.factor(AnimalName)), data=d3, shape = 21,size = 2) +
  geom_line(aes(x = as.factor(TestNumber),y = sum60min, group = strainName), data=d3, size = 1.0, stat = "summary") +
...
sp1 <- ggplot() +
  ...
  geom_line(aes(x = as.factor(TestNumber),y = NetResponse, group = strainName), data=d3, size = 1.0, stat = "summary") +
...
sp1 <- ggplot() +
  ...
  geom_line(aes(x = as.factor(TestNumber),y = AvgResp30min, group = strainName), data=d3, size = 1.0, stat = "summary") +
...
sp1 <- ggplot() +
  ...
  geom_line(aes(x = as.factor(TestNumber),y = BaselineAvg, group = strainName), data=d3, size = 1.0, stat = "summary") +
